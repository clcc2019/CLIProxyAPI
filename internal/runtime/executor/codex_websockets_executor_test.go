...
func TestCodexShouldRetryWithoutPreviousResponseWhenContextCanBeReplayed(t *testing.T) {
	...
	if !codexShouldRetryWithoutPreviousResponse(fullBody, internalIncremental, noCustomToolCallPayload) {
		t.Fatal("internally compressed custom tool output should retry with full transcript when upstream loses the tool call")
	}
...
func TestCodexEnsureVersionHeaderNormalizesMinimum(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			sourceVersion: "0.152.1-alpha.1",
			want:          "0.152.1-alpha.1",
		},
		{
			name:          "newer stable client is preserved",
			sourceVersion: "0.153.0",
			want:          "0.153.0",
		},
		{
