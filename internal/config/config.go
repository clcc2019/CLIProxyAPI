...
type Config struct {
	...
	// CredentialConcurrency contains Home-authoritative credential lifecycle
	// settings. It is kept in the overlay so standalone nodes can validate the
	// same lease/reclaim protocol as Home.
	CredentialConcurrency CredentialConcurrencyConfig `yaml:"credential-concurrency" json:"credential-concurrency"`

	// CredentialInFlight bounds periodic in-flight observation snapshots.
	...
	// claude, codex, kimi, and xai.
	//
	// NOTE: This does not apply to existing per-credential model alias features under:
	// codex-api-key, claude-api-key, and openai-compatibility.
...
type RoutingConfig struct {
	...
	// X-Client-Request-Id (PI), metadata.user_id,
	...
	// SessionAffinitySubagents enables parent-session fallback for forked or
	// subagent requests when no exact LCP/session match exists.
...
type PayloadModelRule struct {
	// Name is the model name or wildcard pattern (e.g., "gpt-*", "claude-*", "*-mini").
	Name string `yaml:"name" json:"name"`
	// Protocol restricts the rule to a specific translator format (e.g., "responses", "claude").
	Protocol string `yaml:"protocol" json:"protocol"`
	// Headers restricts the rule to requests whose headers match all configured wildcard patterns.
	Headers map[string]string `yaml:"headers" json:"headers"`
	// FromProtocol restricts the rule to a specific source protocol (e.g., "responses", "claude").
...
type ClaudeKey struct {
	...
	// RequestScopedErrors configures custom handling rules for matching upstream errors.
...
type CodexKey struct {
	...
	// PoolMode retries errors on the selected Codex API key before falling
	// through to the next pooled model or credential.
	PoolMode bool `yaml:"pool-mode,omitempty" json:"pool-mode,omitempty"`
...
type OpenAICompatibility struct {
	...
	// PoolMode retries errors on the selected OpenAI-compatible provider before
	// falling through to the next pooled model or credential.
	PoolMode bool `yaml:"pool-mode,omitempty" json:"pool-mode,omitempty"`
...
func SaveConfigPreserveComments(configFile string, cfg *Config) error {
	if cfg != nil {
		if err := cfg.ValidateCredentialWeights(); err != nil {
			return fmt.Errorf("validate credential weights: %w", err)
		}
	}
	persistCfg := cfg
