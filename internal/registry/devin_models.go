...
func GetDevinModels() []*ModelInfo {
	if models := GetDevinModelsFromCatalog(); len(models) > 0 {
		return models
	}
...
// GetDevinModelsFromCatalog returns only the separately refreshed catalog.
// It is intentionally distinct from GetDevinModels to avoid recursive fallback.
func GetDevinModelsFromCatalog() []*ModelInfo {
