...
func BuildClaudeResponse(content string, toolUses []KiroToolUse, model string, usageInfo usage.Detail, stopReason string) []byte {
	...
	// Only emit the cache / reasoning fields when upstream actually reported
	// them. Zero-valued fields confuse Claude Code's prompt-cache heuristic
	// into believing the cache was probed and missed.
