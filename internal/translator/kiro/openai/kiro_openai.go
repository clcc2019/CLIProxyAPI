...
func ConvertKiroStreamToOpenAI(ctx context.Context, model string, originalRequest, request, rawResponse []byte, param *any) [][]byte {
	...
	case "message_delta":
		...
		// Extract usage if present. The upstream SSE event carries Claude's
		// full usage shape — input_tokens / output_tokens plus optional
		// cache_read_input_tokens, cache_creation_input_tokens, and
		// reasoning_tokens. Propagate every field so BuildOpenAISSEUsage
		// can surface cached + reasoning counts to OpenAI clients.
		if eventJSON.Get("usage").Exists() {
			usageInfo := kiroUsageFromClaudeJSON(eventJSON.Get("usage"))
...
func ConvertKiroNonStreamToOpenAI(ctx context.Context, model string, originalRequest, request, rawResponse []byte, param *any) []byte {
	...
	// Extract usage from the Claude-format response the Kiro executor
	// produced. Preserves cache_read / cache_creation / reasoning buckets
	// so BuildOpenAIResponseWithReasoning can forward them to clients that
	// read OpenAI's nested details shape.
	usageInfo := kiroUsageFromClaudeJSON(response.Get("usage"))
...
// kiroUsageFromClaudeJSON reads Claude-shaped usage JSON and returns a
// fully-populated usage.Detail, computing TotalTokens when absent. It is
// the single source of truth for how the Kiro→OpenAI translator consumes
// upstream usage frames — keeping it in one place prevents the delta /
// ping / non-stream branches from drifting in what they read.
func kiroUsageFromClaudeJSON(usageJSON gjson.Result) usage.Detail {
	if !usageJSON.Exists() {
		return usage.Detail{}
	}
	detail := usage.Detail{
	...
	// Upstream sometimes omits total_tokens; synthesize one so downstream
	// cost estimators do not see zero.
	if total := usageJSON.Get("total_tokens").Int(); total > 0 {
