...
func TestBuildKiroPayloadFromOpenAIDoesNotReinjectSystemPromptAfterHistory(t *testing.T) {
	input := []byte(`{
	...
	if strings.Contains(content, "--- SYSTEM PROMPT ---") || strings.Contains(content, "Always answer briefly.") || strings.Contains(content, "Current time is") {
		t.Fatalf("current message reinjected system prompt after history: %q", content)
	}
	if !strings.Contains(content, "second turn") {
		t.Fatalf("current message content = %q, want second turn", content)
	}
}
