...
// and integration with various AI API handlers (OpenAI and Claude).
...
func defaultRequestLoggerFactory(cfg *config.Config, configPath string) logging.RequestLogger {
	...
	if cfg == nil {
		return logging.NewFileRequestLogger(false, logsDir, configDir, 0)
	}
...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	if errApplyAccess := s.applyAccessConfig(nil, cfg); errApplyAccess != nil {
		log.Errorf("failed to apply initial access provider config: %v", errApplyAccess)
	}
	if authManager != nil {
...
func (s *Server) setupRoutes() {
	...
	s.engine.GET("/anthropic/callback", s.oauthCallbackRouteHandler("anthropic"))
	s.engine.GET("/codex/callback", s.oauthCallbackRouteHandler("codex"))
	s.engine.GET("/xai/callback", s.oauthCallbackRouteHandler("xai"))

	// Management routes are registered lazily by registerManagementRoutes when a secret is configured.
}

func (s *Server) oauthCallbackRouteHandler(provider string) gin.HandlerFunc {
	return func(c *gin.Context) {
		code := c.Query("code")
		state := c.Query("state")
...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		mgmt.PATCH("/auth-files/codex-auth-mode", s.mgmt.PatchCodexAuthMode)
...
func managementCORSPath(path string) bool {
	...
	return strings.HasPrefix(path, "/v0/management")
}
...
func (s *Server) UpdateClients(cfg *config.Config) {
	if s == nil || cfg == nil {
		log.Warn("ignored nil server configuration update")
		return
	}
	...
	if len(s.oldConfigYaml) > 0 {
		if err := yaml.Unmarshal(s.oldConfigYaml, &oldCfg); err != nil {
			log.Errorf("failed to decode previous config snapshot: %v", err)
			oldCfg = nil
		}
	}

	update := serverConfigUpdate{server: s, old: oldCfg, next: cfg}
	update.applyObservability()
	update.applyAuthRuntime()
	update.applyManagementRoutes()
	update.applyRuntimeSnapshot()
	update.logClientSummary()
}
