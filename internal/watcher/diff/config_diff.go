...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if oldCfg.Streaming.UpstreamDrainAfterDownstreamCancelMS != newCfg.Streaming.UpstreamDrainAfterDownstreamCancelMS {
		changes = append(changes, fmt.Sprintf("streaming.upstream-drain-after-downstream-cancel-ms: %d -> %d", oldCfg.Streaming.UpstreamDrainAfterDownstreamCancelMS, newCfg.Streaming.UpstreamDrainAfterDownstreamCancelMS))
	}
	...
	oldPanelTag := strings.TrimSpace(oldCfg.RemoteManagement.PanelGitHubTag)
	newPanelTag := strings.TrimSpace(newCfg.RemoteManagement.PanelGitHubTag)
	if oldPanelTag != newPanelTag {
		changes = append(changes, fmt.Sprintf("remote-management.panel-github-tag: %s -> %s", oldPanelTag, newPanelTag))
	}
