...
func (w *Watcher) reloadClients(rescanAuth bool, affectedOAuthProviders []string, forceAuthRefresh bool) {
	...
	claudeAPIKeyCount, codexAPIKeyCount, openAICompatCount := BuildAPIKeyClients(cfg)
	totalAPIKeyClients := claudeAPIKeyCount + codexAPIKeyCount + openAICompatCount
...
func BuildAPIKeyClients(cfg *config.Config) (int, int, int) {
	...
	return claudeAPIKeyCount, codexAPIKeyCount, openAICompatCount
}
