...
func TestKiroSocialOAuthBuildLoginURL(t *testing.T) {
	...
	loginURL, err := client.BuildLoginURL("github", "http://localhost:3128", "state-123", "challenge-123")
	if err != nil {
	...
	if parsed.Scheme != "https" || parsed.Host != "example.test" || parsed.Path != "/signin" {
		t.Fatalf("login URL = %s", loginURL)
	}
	q := parsed.Query()
	if q.Get("idp") != "" {
		t.Fatalf("idp = %q, want empty", q.Get("idp"))
	}
	if q.Get("redirect_uri") != "http://localhost:3128" {
		t.Fatalf("redirect_uri = %q", q.Get("redirect_uri"))
	}
	if q.Get("state") != "state-123" || q.Get("code_challenge") != "challenge-123" || q.Get("code_challenge_method") != "S256" {
		t.Fatalf("unexpected query: %v", q)
	}
	if q.Get("redirect_from") != "kirocli" {
		t.Fatalf("redirect_from = %q, want kirocli", q.Get("redirect_from"))
	}
}

func TestKiroSocialOAuthBuildLoginURLOfficialSigninParams(t *testing.T) {
	client := NewSocialOAuthClientWithHTTPClient("https://example.test", nil)
	loginURL, err := client.BuildLoginURLWithOptions("google", "http://localhost:3128", "state-123", "challenge-123", SocialLoginURLOptions{
		ForceReauth: true,
	})
	if err != nil {
	...
	if q.Get("prompt") != "" {
		t.Fatalf("prompt = %q, want empty", q.Get("prompt"))
	}
	if q.Get("max_age") != "" {
...
func TestKiroSocialOAuthExchangeCode(t *testing.T) {
	...
	token, err := client.ExchangeCode(context.Background(), "auth-code", "http://localhost:3128", "verifier")
	if err != nil {
		t.Fatalf("ExchangeCode() error = %v", err)
	}
	if gotPayload["code"] != "auth-code" || gotPayload["code_verifier"] != "verifier" || gotPayload["redirect_uri"] != "http://localhost:3128" {
...
func TestKiroSocialOAuthExchangeCodeRejectsMissingRefreshToken(t *testing.T) {
	...
	client := NewSocialOAuthClientWithHTTPClient(server.URL, server.Client())
	if _, err := client.ExchangeCode(context.Background(), "auth-code", "http://localhost:3128", "verifier"); err == nil {
