...
type disabledTestTokenStorage struct {
	meta map[string]any
}

func (s *disabledTestTokenStorage) SetMetadata(meta map[string]any) { s.meta = meta }

func (s *disabledTestTokenStorage) SaveTokenToFile(authFilePath string) error {
...
func TestFileTokenStore_Save_DisabledPersistsFlagForTokenStorage(t *testing.T) {
	...
	storage := &disabledTestTokenStorage{}
