...
func TestManagerMarkResultSkipsSessionContextErrorEvent(t *testing.T) {
	withEnabledErrorQueue(t)
	subscriber, unsubscribe := redisqueue.SubscribeErrors()
	defer unsubscribe()
	...
	manager := NewManager(nil, nil, nil)
	auth := &Auth{
		ID:       "auth-session-context-error-event",
		Provider: "codex",
		Metadata: map[string]any{
			"type": "codex",
		},
	...
	manager.MarkResult(context.Background(), Result{
		...
		Success:  false,
		Error: &Error{
			Message:    "HTTP 400: Previous response with id 'resp_04bda3d65722fdcd016a2116fb735081918d838adf45075eab' not found.",
	...
	select {
	case got := <-subscriber:
		t.Fatalf("received session-context error event %q, want none", string(got))
