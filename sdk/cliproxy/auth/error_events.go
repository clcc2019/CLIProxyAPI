...
func (m *Manager) publishErrorEvent(result Result, authSnapshot *Auth) {
	if !m.shouldPublishErrorEvent(result) || authSnapshot == nil {
		return
	}
...
func (m *Manager) shouldPublishErrorEvent(result Result) bool {
	if m == nil || result.Success || m.HomeEnabled() || !redisqueue.Enabled() {
		return false
	}
	return !isSessionContextResultError(result.Error)
}
